<?php
    ...
    class DeviceLocation extends WCTPOperation
    {
        // Required
        ...
        public function messageControlOptions( array $options )
        {
            ...
            $this->fixType = $options['fixType'] ?? null;
        ...
        public function xml(): SimpleXMLElement
        {
            ...
            $deviceLocation = $xml->addChild( 'wctp-DeviceLocation' );

            $deviceLocation->addAttribute( 'submitTimestamp', $this->submitTimestamp->timezone('UTC')->format( 'Y-m-d\TH:i:s' ) );

            $originator = $deviceLocation->addChild('wctp-Originator' );
            ...
            $recipient = $deviceLocation->addChild( 'wctp-Recipient' );
            ...
            $messageControl = $deviceLocation->addChild('wctp-DeviceLocationControl' );
            ...
            if( $this->fixType ) { $messageControl->addAttribute( 'fixType', $this->fixType ); }

            if( $xml === false || $deviceLocation === false || $originator === false || $recipient === false || $messageControl === false )
            {
        ...
        private function validate()
        {
            ...
            elseif( ! is_null( $this->fixType ) &&
                ! (
                    $this->fixType == FixType::APPROXIMATE ||
                    $this->fixType == FixType::ESTIMATED ||
                    $this->fixType == FixType::PRECISE
                ))
            {
                $msg = 'fixType must be one of APPROXIMATE, ESTIMATED, or PRECISE';
            }
